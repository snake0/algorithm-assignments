...
\textbf{Solution:}
    \begin{itemize}
        ...
        \item Else, contract each connected component in $B$ to a single vertex. Create a new graph by using all vertexes from the contraction ($V'$), and all edges in $A$ that connects the connected components in $B$ ($E'$). We have $|V'|=|F|$, $G'=(V',E')$, MBST is $F \cup MBST(G')$.
    \end{itemize}
    \begin{algorithm}[htb]
